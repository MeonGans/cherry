        ...
        <form action="{{ route('sessions.store') }}" method="POST" class="grid gap-5 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
                ...
                <textarea
                    ...
                    rows="10"
                    class="form-textarea"
                    placeholder="Іванов Іван - 5 - ч&#10;Петренко Марія - 2"
                >{{ old('students') }}</textarea>
                <p class="mt-2 text-xs text-white-dark">
                    Формат: ім'я учня - ID очікуваної команди - ч. Позначку «ч» додавайте для хлопців;
                    без неї учень буде доданий як дівчина. Для кожного учня автоматично створиться PIN-код.
                </p>
            </div>
